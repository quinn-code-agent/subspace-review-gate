...
def relay_disable_room(args, **_):
    command = ["disable-room", "--briefing", args["briefing"], "--room-id", args["room_id"]]
    if args.get("endpoint"): command.extend(["--endpoint", args["endpoint"]])
    if args.get("state_dir"): command.extend(["--state-dir", args["state_dir"]])
    return relay_call(*command)
...
def relay_revoke_room_session(args, **_):
    command = ["revoke-room-session", "--briefing", args["briefing"], "--room-id", args["room_id"], "--session-id", args["session_id"]]
    if args.get("endpoint"): command.extend(["--endpoint", args["endpoint"]])
    if args.get("state_dir"): command.extend(["--state-dir", args["state_dir"]])
    return relay_call(*command)
...
def register(ctx):
    ...
    ctx.register_tool(name="subspace_review_gate_relay_pull_result", toolset="subspace_review_gate", emoji="📥", check_fn=available,
        schema=schema("subspace_review_gate_relay_pull_result", "Owner-only pull and local SHA-256 verification of one feedback-only Relay Result; this does not create a Resolution or change workflow state.", {**common, "result_id": {"type": "string"}, "output_dir": {"type": "string"}, **relay_endpoint, "state_dir": {"type": "string", "description": "Optional private owner-receipt directory."}}, ["briefing", "result_id", "output_dir"]), handler=relay_pull_result)
    ...
    ctx.register_tool(name="subspace_review_gate_relay_disable_room", toolset="subspace_review_gate", emoji="🛑", check_fn=available,
        schema=schema("subspace_review_gate_relay_disable_room", "Disable an existing Relay Review Room for all reviewer sessions using its private owner receipt. This does not write a workflow verdict.", {**room_props, "room_id": {"type": "string"}}, ["briefing", "room_id"]), handler=relay_disable_room)
    ctx.register_tool(name="subspace_review_gate_relay_revoke_room_session", toolset="subspace_review_gate", emoji="🚫", check_fn=available,
        schema=schema("subspace_review_gate_relay_revoke_room_session", "Revoke one arrived reviewer session in a Relay Room using its private owner receipt. This does not issue or revoke invitations and does not write workflow state.", {**room_props, "room_id": {"type": "string"}, "session_id": {"type": "string"}}, ["briefing", "room_id", "session_id"]), handler=relay_revoke_room_session)
