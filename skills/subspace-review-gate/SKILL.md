...
3. **Choose the review surface.**
   ...
   - For a Relay-hosted browser review, use `subspace_review_gate_relay_package`, `subspace_review_gate_relay_publish`, then the owner-client tools to create a Room. Use `subspace_review_gate_relay_results` only for agent-safe structural metadata; it deliberately withholds reviewer labels and comments. Use `subspace_review_gate_relay_owner_inbox` to write a private script-free HTML snapshot for the human owner, and do not read that free text into agent context by default. Use `subspace_review_gate_relay_pull_result` only when exact validated bytes are needed locally. Do not host the reviewer browser yourself or expose the Room capability automatically.
