...
3. **Choose the review surface.**
   ...
   - For a Relay-hosted browser review after a disclosed clear Yes, use `subspace_review_gate_relay_share_consented`. It fail-closes on changed/unknown/sensitive/unsupported facts and sequences `subspace_review_gate_relay_package`, `subspace_review_gate_relay_publish`, and Room creation while returning only a safe Room URL plus Relay's actual expiry. Do not host the browser yourself.
   - Start `subspace_review_gate_relay_watch_feedback` with the dispatch-bound origin channel/thread. It runs in the background, deduplicates Result IDs through private cursor state, validates exact pulled bytes, and emits only a safe advisory event. Stop it explicitly when requested; Room disable, Briefing expiry, and configured first-valid-feedback mode also stop it.
